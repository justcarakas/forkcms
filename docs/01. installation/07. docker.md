...
*If this is an existing project, remove your `config/parameters.yaml`. This ensures that we can install a 
...
\* *We store the `var/log` and `/var/cache` folders in their own named volume. This means that the Symfony logs and cache
...
Note: you can also create a `docker-compose.override.yaml` and apply your changes here. Docker automatically reads this file
...
Example: for Redis, simply add a new Docker service to your `docker-compose.yml` or `docker-compose.override.yaml` file:
...
The `var/cache` and `var/log` folders are never modified by the user directly. There's no benefit in
