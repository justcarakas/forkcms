...
final class MetaLink
{
    ...
    public static function alternateLocale(string $href, string|Locale $locale): self
    {
        return new self(
            $href,
            ['rel' => 'alternate', 'hreflang' => $locale instanceof Locale ? $locale->value : $locale]
        );
    }
    ...
    public function __toString(): string
    {
        ...
        $html .= implode(
            ' ',
            array_map(
                static function (string $parameterKey, string $parameterValue): string {
